...
// Helper function to check if a table spans multiple pages
const checkTablePageSpan = (tableBounds: DOMRect, containerBounds: DOMRect, pageHeight: number): {
  spansMultiplePages: boolean,
  startPage: number,
  endPage: number,
  pageBreaks: number[]
} => {
  const relativeTop = tableBounds.top - containerBounds.top;
  const relativeBottom = relativeTop + tableBounds.height;
    ...
    const measureAndPaginate = async () => {
      ...
      setTimeout(() => {
        if (measureRef.current) {
          ...
          // Check tables for page spans
          const tableBounds = Array.from(measureRef.current.querySelectorAll('[data-skrift-table="true"]'))
            .map(el => el.getBoundingClientRect());
          console.log("tableBounds",  tableBounds)
          const spanningTables = tableBounds
            .map((bounds, index) => ({
              index,
              bounds,
              spanInfo: checkTablePageSpan(bounds, containerBounds, effectivePageHeight),
              tableWithHeader: tablesWithHeaders[index]
            }))
