...
describe('containsEmailTemplate()', () => {
    ...
    subDirectories: [
      {
        ...
        subDirectories: [
          {
            absolutePath: '/fake/path/emails/magic-links/skrift',
  ...
  it('should work with email inside a second sub directory', () => {
    expect(
      containsEmailTemplate('magic-links/skrift/verify-email', directory),
    ).toBe(true);
    expect(
      containsEmailTemplate('magic-links/skrift/missing-template', directory),
    ).toBe(false);
  });
});
