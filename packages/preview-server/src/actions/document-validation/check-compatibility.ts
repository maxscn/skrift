...
import {
  ...
  getObjectVariables,
  type SourceLocation,
} from '../../utils/canidocument/ast/get-object-variables';
import type { StylePropertyUsage } from '../../utils/canidocument/ast/get-used-style-properties';
import {
  doesPropertyHaveLocation,
  getUsedStyleProperties,
} from '../../utils/canidocument/ast/get-used-style-properties';
import type {
  CompatibilityStats,
  SupportStatus,
} from '../../utils/canidocument/get-compatibility-stats-for-entry';
import { getCompatibilityStatsForEntry } from '../../utils/canidocument/get-compatibility-stats-for-entry';
import { getCssFunctions } from '../../utils/canidocument/get-css-functions';
import { getCssPropertyNames } from '../../utils/canidocument/get-css-property-names';
import { getCssPropertyWithValue } from '../../utils/canidocument/get-css-property-with-value';
import { getCssUnit } from '../../utils/canidocument/get-css-unit';
import { getElementAttributes } from '../../utils/canidocument/get-element-attributes';
import { getElementNames } from '../../utils/canidocument/get-element-names';
import { supportEntries } from './canidocument-data';
...
export interface CompatibilityCheckingResult {
  ...
  statsPerDocumentClient: CompatibilityStats['perDocumentClient'];
}
      ...
      Partial<
        Record<
          ...
          /*
            ...
            ordered version of canidocument's data is meant to be something like:
...
const relevantDocumentClients: DocumentClient[] = [
  'gmail',
  'apple-mail',
  'outlook',
  'yahoo',
    ...
    async start(controller) {
      for (const entry of supportEntries) {
        const compatibilityStats = getCompatibilityStatsForEntry(
          entry,
          relevantDocumentClients,
        );
        if (Object.keys(compatibilityStats.perDocumentClient).length === 0)
          continue;
        if (
